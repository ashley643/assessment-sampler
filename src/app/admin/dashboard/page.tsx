          ...
          <>
              ...
              <Section title="Daily Visits">
                <div className="p-4 space-y-3">
                  {data.daily.map((d, idx) => {
                    ...
                    const isBottom = idx >= Math.ceil(data.daily.length / 2);
                    return (
                      <div key={d.date} className="flex items-center gap-3 text-sm group relative">
                        ...
                        {sorted.length > 0 && (
                          <div className={`absolute left-16 z-20 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg px-3 py-2 shadow-xl whitespace-nowrap ${isBottom ? 'bottom-full mb-1' : 'top-full mt-1'}`}>
                            {sorted.map(([c, n]) => (
            ...
            {Object.keys(data.byBundle).length > 0 && (
              <Section title="Activity by Bundle">
                <div className="p-4 space-y-3">
                  {(() => {
                    const bundleEntries = Object.entries(data.byBundle).sort((a, b) => b[1] - a[1]);
                    const max = Math.max(...Object.values(data.byBundle));
                    return bundleEntries.map(([name, count], idx) => {
                      const pct = max > 0 ? (count / max) * 100 : 0;
                      const byCode = data.bundleByCode?.[name] ?? {};
                      const sorted = Object.entries(byCode).sort((a, b) => b[1] - a[1]);
                      const isBottom = idx >= Math.ceil(bundleEntries.length / 2);
                      return (
                        <div key={name} className="flex items-center gap-3 text-sm group relative">
                          ...
                          {sorted.length > 0 && (
                            <div className={`absolute left-44 z-20 hidden group-hover:block bg-gray-900 text-white text-xs rounded-lg px-3 py-2 shadow-xl whitespace-nowrap ${isBottom ? 'bottom-full mb-1' : 'top-full mt-1'}`}>
                              {sorted.map(([c, n]) => (
                                <div key={c} className="flex items-center justify-between gap-4">
                                  <span className="font-mono text-gray-300">{c}</span>
                                  <span className="font-semibold">{n}</span>
