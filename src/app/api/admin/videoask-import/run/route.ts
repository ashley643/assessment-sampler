...
export type RunResult =
  ...
  | { wouldInsert: number; wouldSkip: number; totalStepsFetched: number; sample: Record<string, unknown>[]; rowsWithMetadata?: number; error?: never }
  | { error: string };
...
export async function runImportCore(params: RunParams): Promise<RunResult> {
  ...
  if (dryRun) {
    // Prefer sample rows that have metadata fields populated so the preview is more informative
    const metadataKeys = nodeRoles
      ? Object.values(nodeRoles)
          .filter((r): r is Extract<NodeRole, { role: 'metadata' }> => r.role === 'metadata')
          .map(r => r.targetColumn)
      : [];
    const withMeta = metadataKeys.length > 0
      ? toInsert.filter(r => metadataKeys.some(k => r[k] != null && r[k] !== ''))
      : [];
    const sample = withMeta.length >= 3 ? withMeta.slice(0, 3)
      : withMeta.length > 0 ? [...withMeta, ...toInsert.filter(r => !withMeta.includes(r))].slice(0, 3)
      : toInsert.slice(0, 3);
    return { wouldInsert: toInsert.length, wouldSkip: skipped, totalStepsFetched: steps.length, sample, rowsWithMetadata: withMeta.length };
  }
