...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { title, type, type_label, accent_color, badge_bg, badge_text, description, question, player_label, sort_order, questions } = body;

  const { error } = await supabaseAdmin
    .from('assessments')
    .update({ title, type, type_label, accent_color, badge_bg, badge_text, description, question: question || null, player_label: player_label || null, sort_order })
    .eq('id', id);
    ...
    if (questionRows.length) {
      ...
      const sampleRows = (questions as IncomingQuestion[]).flatMap((q, qi) =>
        (q.question_samples ?? []).filter(s => s.embed_url.trim()).map((s, si) => ({
          ...
          language: s.language,
          media_type: (s.media_type && s.media_type !== '') ? s.media_type : 'video',
