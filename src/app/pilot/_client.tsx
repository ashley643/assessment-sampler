...
const LP_ATTRIBUTES: LPAttribute[] = [
  ...
  // ── Other Common Portrait Competencies (alphabetical) ───────────────────────
  ...
  { id: 'pog-confidence',    name: 'Confidence',                            group: 'portrait', questionIds: ['el-6','el-9','sec-5','sec-9','sec-10'] },
  ...
  { id: 'pog-kindness',      name: 'Kindness',                              group: 'portrait', questionIds: ['el-13','el-14','sec-13','sec-14'] },
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', background: '#f4f7fc' }}>
          <div id="lib-doc" style={{ background: 'white', borderRadius: 12, boxShadow: '0 2px 16px rgba(0,0,0,0.06)', maxWidth: 700, margin: '0 auto', overflow: 'hidden' }}>
                ...
                {bh.questions.map((q, i) => (
                  <div key={i} style={{ borderLeft: '3px solid #dce8f5', paddingLeft: 14, marginBottom: 12, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                      <span style={{ fontSize: 10, fontWeight: 600, color: '#d1d5db', minWidth: 18, textAlign: 'right', flexShrink: 0, paddingTop: 1 }}>{i + 1}</span>
            ...
            {CS_ALL_GROUPS.map(({ key, label }) => {
              ...
              const csAllForKey = CS_QUESTIONS.filter(q => q.age === key);
              const csNum = (q: { id: string }) => csAllForKey.findIndex(q2 => q2.id === q.id) + 1;
...
function SaveForLaterModal({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const csAllForKey = csKey ? CS_QUESTIONS.filter(q => q.age === csKey) : [];
  const csNum = (q: { id: string }) => csAllForKey.findIndex(q2 => q2.id === q.id) + 1;
